      ...
      MongoClient.connect(_self.options.uri, function(err, db) {
        ...
        db.createCollection(_self.options.collection, function(err, collection) {
          ...
          collection.findOne({language: language, namespace: namespace}, function (err, lang) {
            ...
            /*if(lang)
              return callback(null, lang.data);

            collection.insert({language: language, namespace: namespace, data: {}}, function (err, results) {
              if(err) return callback(err);
    ...
    value: function create(languages, namespace, key, fallbackValue, callback) {
      ...
      MongoClient.connect(_self.options.uri, function(err, db) {
            ...
            languages.forEach(function (lng) {
              collection.update({language: lng, namespace: namespace}, {$set: set}, {upsert: true}, function (err) {
                if(err) return console.error(err);
              });
            });
					});
